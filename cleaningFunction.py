...
def cleaning_dump_extract_csv(path_to_nutch, path_to_csv, path_to_java_home, path_to_dumps):
    ...
    list_of_segments = sorted(Path(f'{path_to_nutch}/nutch/segments').iterdir(), key=os.path.getmtime)
        ...
        for segment in tqdm.tqdm(segments_to_process):
            try:
                ...
                if df.shape[0] > 0:
                    ...
                    logger_txt = f'Segment: {dump} | Date: {start.strftime("%Y-%m-%d %H:%M:%S")} | Tokens all: {all_tokens} | Tokens cleaned: {cleaned_tokens}'
                ...
                logger.error(f'The dump is broken {segment}')
                error_message = f'The dump is broken {segment}'
                with open('the_segment_with_error.txt', 'a') as txt_file:
                    txt_file.write("%s\n" % error_message)
        end = datetime.now()
